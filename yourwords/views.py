...
from .services import random_record, user_has_option, set_pagination, querysetToJsonLike
...
def index(request):
	...
	if record_count > 0:
		...
		records_to_json = {
			...
			'record_list': querysetToJsonLike(record_list)
		}
...
def search(request):
	...
	if request.method == 'POST':
		...
		if user_has_option(request.user, 'extended_searching'):
			if not 'searched_text' in request.session:
...
def listings(request):
	context = {}
	listings = Listing.users.where_user(request.user).order_by('-created_at')
	context['listings'] = listings
	return render(request, 'yourwords/loads/listings.html', {'listings': listings})


def about(request):
	context = {}
	context['title'] = _('O aplikacji')
	return render(request, 'yourwords/about.html', context)


def cookies(request):
	context = {}
	context['title'] = _('Polityka cookies')
